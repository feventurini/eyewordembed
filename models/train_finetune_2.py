...
if __name__ == '__main__':
    ...
    if model_eyetracking_inference == 'linreg':
        ...
        train_iter = EyetrackingBatchIterator(train, window, batchsize_eyetracking, repeat=True, shuffle=True, wlen=wlen, pos=pos, prev_fix=prev_fix)
        val_iter = EyetrackingBatchIterator(val, window, batchsize_eyetracking, repeat=False, shuffle=True, wlen=wlen, pos=pos, prev_fix=prev_fix)
    ...
    model.save(os.path.join(out_folder, 'finetuned_multitask_gigaword_{}_{}_eyetracking_'.format(os.path.basename(train_tarball), n_units)) + 
