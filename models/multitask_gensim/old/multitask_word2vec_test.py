...
class MultiTaskWord2Vec(Word2Vec):
     ...
     def train(self, sentences, multitask_dataset, multitask_optimizers, total_examples=None, total_words=None,
        ...
        def worker_loop(optimizer):
            ...
            while True:
                ...
                sentences, multitask_batch, alpha = job
                tally, raw_tally = self._do_train_job(sentences, alpha, (work, neu1))

                #input("Thread: " + str(threading.get_ident()) + " BEFORE:\n" + str(self.wv.syn0))
                multitask_model = optimizer.target
                multitask_model.embed.W.data = copy.deepcopy(self.wv.syn0)
                old_syn0 = copy.deepcopy(multitask_model.embed.W.data)

                x_array, t_array = multitask_batch
                x = chainer.Variable(x_array)
                t = chainer.Variable(t_array)
                optimizer.update(multitask_model, x, t)

                update = multitask_model.embed.W.data - old_syn0

                self.wv.syn0 += update
                #input("Thread: " + str(threading.get_ident()) + " AFTER:\n" + str(self.wv.syn0))
        ...
        workers = [threading.Thread(target=worker_loop, args=(o,)) for _,o in zip(xrange(self.workers), multitask_optimizers)]
