...
class ProgressBarWord2Vec(extension.Extension):
    ...
    def __init__(self, word2vec_extension, training_length=None, update_interval=100,
        ...
        self._out = out
        self._recent_timing = []
        self.word2vec_extension = word2vec_extension
    ...
    def __call__(self, trainer):
        ...
        if iteration % self._update_interval == 0:
            ...
            if len(recent_timing) > 1:
                elapsed = (now - recent_timing[len(recent_timing) - 2][2])
                speed = int(self.word2vec_extension.trained_word_count / elapsed)
            else:
                speed = float('inf')               
 
            out.write('{} words/sec. Estimated time to finish: {}.\n'
                     .format(speed, datetime.timedelta(seconds=estimated_time)))
